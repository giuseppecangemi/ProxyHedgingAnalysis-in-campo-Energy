...
def conditional_OLS_hedge_ratio(spot, future, log):
    ...
    data['Base_alpha'] = (data['Gas_Spot_returns'].shift(1) - data['Future_returns'].shift(1))
    ...
    X = data_cleaned[['Base_alpha','Future_returns', 'Future_returns_lag', 'Base_lag']]
    ...
    #CONTROLLARE PAG 27 PAPER MAFFRE E SVILUPPARE LA STESSA IMPOSTAZIONE
    #la stima di beta_0 è statisticamente significativa, anche se gli altri parametri no. Come si procede in questo caso? in genere dovremmo dire che il modello non è ben specificato ma nel paper si dice di testare principalmente b_0 
